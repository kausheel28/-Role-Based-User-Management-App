...
interface EditCallModalProps {
  isOpen: boolean;
  onClose: () => void;
  call: any; // The call to edit
}

const EditCallModal: React.FC<EditCallModalProps> = ({ isOpen, onClose, call }) => {
  const [formData, setFormData] = useState<EditCallForm>({
    caller_name: '',
    caller_number: '',
  ...
  const updateCallMutation = useMutation({
    mutationFn: (data: EditCallForm) => callsApi.updateCall(call.id, data),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast.error(error.response?.data?.detail || 'Failed to update call');
    },
  });

  // Initialize form with call data when modal opens
  useEffect(() => {
    if (isOpen && call) {
      setFormData({
  ...
  const handleInputChange = (field: keyof EditCallForm, value: string | number | boolean | CallType | CallStatus) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateCallMutation.mutate(formData);
  };
        ...
        <div className="mt-3">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              ...
              <h3 className="ml-3 text-lg font-medium text-gray-900">Edit Call</h3>
            </div>
            <button
              onClick={onClose}
